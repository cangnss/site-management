package utils;

public class Menu {
	
	public static void createMainMenu() {
		System.out.println("---------------");
		System.out.println("1 - Apartment Process");
		System.out.println("2 - Person Process");
		System.out.println("3 - Role Process");
		System.out.println("4 - Subscription Process");
		System.out.println("5 - Expense Process");
		System.out.println("6 - Meeting Process");
		System.out.println("7 - Decision Process");
		System.out.println("0 - Exit");
		System.out.println("---------------");
	}
	
	public static void createSubMenuApartment() {
		System.out.println("---------------");
		System.out.println("1 - List");
		System.out.println("2 - Add");
		System.out.println("3 - Update");
		System.out.println("4 - Delete");
		System.out.println("5 - Get Apartment");
		System.out.println("6 - Get Resident In Apartment");
		System.out.println("7 - Get Monthly Payment From Apartment");
		System.out.println("8 - Total Income From Apartments");
		System.out.println("0 - Exit");
		System.out.println("---------------");
	}
	
	public static void createSubMenuDesicion() {
		System.out.println("---------------");
		System.out.println("1 - List");
		System.out.println("2 - Add");
		System.out.println("3 - Update");
		System.out.println("4 - Delete");
		System.out.println("5 - Get Total Votes Confirmed and Rejected");
		System.out.println("6 - Get Number Of Votes Confirmed and Rejected By Persons");
		System.out.println("0 - Exit");
	}
	
	public static void createSubMenuFlatNo() {
		System.out.println("---------------");
		System.out.println("1 - List");
		System.out.println("2 - Add");
		System.out.println("3 - Update");
		System.out.println("4 - Delete");
		System.out.println("5 - Get FlatNo By ApartmentId And FlatId");
		System.out.println("6 - Get FlatNo By Number");
		System.out.println("0 - Exit");
		System.out.println("---------------");
	}
	
	public static void createSubMenuMeeting() {
		System.out.println("---------------");
		System.out.println("1 - List");
		System.out.println("2 - Add");
		System.out.println("3 - Update");
		System.out.println("4 - Delete");
		System.out.println("0 - Exit");
	}
	
	
	public static void createSubMenuExpense() {
		System.out.println("---------------");
		System.out.println("1 - List");
		System.out.println("2 - Add");
		System.out.println("3 - Update");
		System.out.println("4 - Delete");
		System.out.println("5 - Get Expenses Apartment");
		System.out.println("0 - Exit");
		System.out.println("---------------");
	}
	
	public static void createSubMenuPerson() {
		System.out.println("---------------");
		System.out.println("1 - List");
		System.out.println("2 - Add");
		System.out.println("3 - Update");
		System.out.println("4 - Delete");
		System.out.println("5 - Get Person");
		System.out.println("0 - Exit");
	}
	
	public static void createSubMenuRole() {
		System.out.println("---------------");
		System.out.println("1 - List");
		System.out.println("2 - Add");
		System.out.println("3 - Update");
		System.out.println("4 - Delete");
		System.out.println("5 - Get Manager In Apartment");
		System.out.println("0 - Exit");
	}
	
	public static void createSubMenuSubscription() {
		System.out.println("---------------");
		System.out.println("1 - List");
		System.out.println("2 - Add");
		System.out.println("3 - Update");
		System.out.println("4 - Delete");
		System.out.println("5 - Get Subscription apartment");
		System.out.println("0 - Exit");
		System.out.println("---------------");
	}
	
	public static void createSubMenuAccount() {
		System.out.println("---------------");
		System.out.println("1 - List");
		System.out.println("2 - Add");
		System.out.println("3 - Update");
		System.out.println("4 - Delete");
		System.out.println("5 - List");
	}
}
